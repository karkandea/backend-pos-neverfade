...
## Checkpoint 14 — onboarding QA promotion and exact-release smoke (2026-09-26)

- Promoted the checklist binary/assets to **isolated QA** release `20260926-s1-onboarding`: backend `63b955edca9323f96dd3691820fe9651ce4067bf`, frontend `45ff6698f6a103b38e57c8aea7f78339013e3bb0`. Previous laundry QA service/Nginx configs and release retained for rollback; QA service active and OpenAPI HTTP 200. No schema migration or production change.
- For exact build coverage, copied frontend assets **from deployed QA release** into a localhost-only static mirror and reverse-proxied same-origin `/api` to the actual running QA API/PostgreSQL through SSH loopback. Verified deployed JS bundle name matched QA release. Real Playwright smoke **51/51 PASS across desktop/tablet/mobile**: original roles/categories, restaurant kitchen, two-outlet/tenant denial, dedicated kitchen/laundry staff redaction and restricted UX, plus new real role-gated onboarding (restaurant/laundry/fashion). This proves deployed frontend+QA API paths, but is **not a public HTTPS Basic-auth-gate test** or full formal UAT.
- Separately probed the five finance read endpoints on this QA: summary, withdrawals, movements and settings HTTP **200**; unconfigured bank account correctly HTTP **204 No Content**. The owner finance UI did not render `Data keuangan belum dapat dimuat` (**1/1 smoke PASS**). This **does not reproduce or close R1 in production**; production incident traces are still required. No banking/payment mutation was executed.
- Backend source verification remains **176/176 PASS** and mocked Sprint 1 UI **24/24 PASS** across three viewport profiles. GitHub Actions remote runs remain blocked by account billing lock. Demo reset, deeper authorization audit, end-to-end payment and formal release sign-off remain open.
