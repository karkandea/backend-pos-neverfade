...
if (string.IsNullOrWhiteSpace(jwtKey) || jwtKey.Length < 32)
    throw new InvalidOperationException("Jwt:Key missing or shorter than 32 characters.");
...
var platformJwtKey = builder.Configuration["PlatformJwt:Key"];
if (string.IsNullOrWhiteSpace(platformJwtKey) || platformJwtKey.Length < 32)
    throw new InvalidOperationException("PlatformJwt:Key missing or shorter than 32 characters.");

var platformJwtIssuer = builder.Configuration["PlatformJwt:Issuer"];
if (string.IsNullOrWhiteSpace(platformJwtIssuer))
    throw new InvalidOperationException("PlatformJwt:Issuer missing.");

var platformJwtAudience = builder.Configuration["PlatformJwt:Audience"];
if (string.IsNullOrWhiteSpace(platformJwtAudience))
    throw new InvalidOperationException("PlatformJwt:Audience missing.");

if (platformJwtKey == jwtKey || platformJwtIssuer == jwtIssuer || platformJwtAudience == jwtAudience)
    throw new InvalidOperationException("Platform JWT key, issuer, and audience must be separate from tenant JWT configuration.");
...
builder.Services.AddHttpClient<IXenditSandboxSimulator, XenditSandboxSimulator>(client =>
{
    client.BaseAddress = new Uri("https://api.xendit.co/");
    client.Timeout = TimeSpan.FromSeconds(30);
});
...
builder.Services.AddScoped<IPlatformJwtService, PlatformJwtService>();
builder.Services.AddScoped<IPlatformAuthService, PlatformAuthService>();
builder.Services.AddScoped<PlatformUserBootstrapService>();
...
builder.Services.AddScoped<ISharedPosJwtService, SharedPosJwtService>();
builder.Services.AddScoped<IEmployeeSharedAccessService, EmployeeSharedAccessService>();
...
builder.Services
    ...
    .AddJwtBearer(PlatformAuthConstants.AuthenticationScheme, options =>
    {
        options.TokenValidationParameters = new TokenValidationParameters
        {
            ...
            IssuerSigningKey = new SymmetricSecurityKey(Encoding.UTF8.GetBytes(platformJwtKey))
        };
        ...
        options.Events = new JwtBearerEvents
        {
            OnTokenValidated = context =>
            {
                if (context.Principal?.HasClaim(claim => claim.Type == "tenant_id") == true)
                    context.Fail("Platform token must not contain tenant_id.");
                return Task.CompletedTask;
            },
...
{
    options.AddPolicy(PlatformAuthConstants.AuthorizationPolicy, policy =>
    {
        policy.AddAuthenticationSchemes(PlatformAuthConstants.AuthenticationScheme);
        policy.RequireAuthenticatedUser();
        policy.RequireClaim(PlatformAuthConstants.ScopeClaim, PlatformAuthConstants.PlatformScope);
        policy.RequireRole(PlatformAuthConstants.SuperAdminRole);
        policy.RequireAssertion(context => !context.User.HasClaim(claim => claim.Type == "tenant_id"));
    });
});
...
await using (var bootstrapScope = app.Services.CreateAsyncScope())
{
    await bootstrapScope.ServiceProvider
        .GetRequiredService<PlatformUserBootstrapService>()
        .InitializeAsync();
}
